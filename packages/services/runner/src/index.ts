...
function sanitizeProjectDirName(input: string): string {
...
function wrapLog(log: LogFn | undefined, scope: string): LogFn | undefined {
...
export class Runner {
  ...
  public readonly workspaceHostPath: string;
  ...
  static defaultProject(projectName = "default"): Runner {
  ...
  async up(log?: LogFn): Promise<void> {
  ...
  async exec(command: string, log?: LogFn): Promise<ExecResult> {
  ...
  async down(log?: LogFn): Promise<void> {
    try {
