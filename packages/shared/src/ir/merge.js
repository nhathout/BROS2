...
function mergeTopicRefs(target, incoming) {
    if (!incoming?.length) {
        return target;
    }
    const seen = new Map();
    for (const ref of [...target, ...incoming]) {
        if (!ref || typeof ref.topic !== "string" || typeof ref.type !== "string")
            continue;
...
export function mergeIRFragments(fragments) {
    ...
    for (const fragment of fragments) {
        if (!fragment || typeof fragment !== "object")
            continue;
        const packages = fragment.packages;
    ...
    const mergedPackages = Array.from(packageMap.values())
        .map((pkg) => {
        const nodes = Array.from(pkg.nodes.values())
            .map((node) => ({
            id: node.id,
            name: node.name,
            package: pkg.name,
